...
class _EyeTrackingScreenState extends State<EyeTrackingScreen> {
  ...
  Future<void> _initEyedid() async {
    ...
    try {
      ...
      if (result.result) {
        ...
        setState(() => _statusMessage = "Initialization failed: ${result.message}");
      }
    } catch (e) {
  ...
  void _listenEvents() {
    _eyedidFlutterPlugin.getTrackingEvent().listen((event) async {
      ...
      if (trackingState == TrackingState.success) {
        const double alpha = 0.2; // Decreased for more smoothing
      ...
      setState(() {
        ...
        _currentScreenState = screenState == ScreenState.insideOfScreen
            ? "inside"
        ...
        if (trackingState == TrackingState.success && _isTracking) {
          ...
          _statusMessage = "Gaze not detected";
      ...
      if (_isTracking && DateTime.now().difference(_lastUpdate).inMilliseconds >= 34) {
        try {
          await _channel.invokeMethod('updateGaze', {
            'x': trackingState == TrackingState.success ? _smoothedX : _lastValidX,
            'y': trackingState == TrackingState.success ? _smoothedY : _lastValidY,
  ...
  @override
  void dispose() {
    if (_isTracking) _eyedidFlutterPlugin.stopTracking();
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Eye Tracking'), backgroundColor: Colors.blueAccent),
      body: Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  const Text('Eye Tracking:', style: TextStyle(fontSize: 18)),
                  const SizedBox(width: 10),
                  Switch(
                    value: _isTracking,
                    onChanged: _hasCameraPermission && _isInitialized
                        ? (value) => value ? _startOverlay() : _stopOverlay()
                        : null,
              ...
              const SizedBox(height: 20),
              Text('Gaze: (${_smoothedX.toStringAsFixed(1)}, ${_smoothedY.toStringAsFixed(1)})'),
              const SizedBox(height: 20),
              ElevatedButton(
